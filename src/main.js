...
function checkCollisions(){
  ...
  mazeGroup.children.forEach(c=>{
    ...
    // Simple push back on smallest overlap
...
function animate(){
  ...
  const dt=0.016;
  const force=1.5; // ← moins sensible
  if(ballMesh){
...
document.getElementById('load-btn').addEventListener('click',()=>{
  ...
  if(url){
    gltfLoader.load(url,gltf=>{
      ...
      const s=Math.max(box.getSize(new THREE.Vector3()).x,box.getSize(new THREE.Vector3()).y,box.getSize(new THREE.Vector3()).z)||1;
      ballMesh.scale.setScalar(ballRadius*2/s);
      scene.add(ballMesh);
      ballMesh.position.copy(ballPos);
    },undefined,err=>{
      console.warn(err);
